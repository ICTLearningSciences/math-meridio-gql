...
const removeStudentFromClassQuery = `
...
describe("remove student from class", () => {
  let app: Express;
  ...
  it(`instructor can remove a student from their classroom`, async () => {
    const response = await request(app)
      .post("/graphql")
      .set("Authorization", `Bearer ${instructorAccessToken}`)
      .send({
        query: removeStudentFromClassQuery,
        variables: {
    ...
    expect(response.body.data.removeStudentFromClass).to.have.property(
      "classId"
    );
    expect(response.body.data.removeStudentFromClass).to.have.property(
      "userId"
    );
    expect(response.body.data.removeStudentFromClass.status).to.equal(
      ClassMembershipStatus.REMOVED
    );
    expect(response.body.data.removeStudentFromClass.userId).to.equal(
      studentUserId
    );
  });
  ...
  it(`fails if a student tries to remove another student`, async () => {
    const response = await request(app)
      .post("/graphql")
