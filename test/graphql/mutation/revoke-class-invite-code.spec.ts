...
const revokeClassInviteCodeQuery = `
...
describe("revoke a class invite code", () => {
  let app: Express;
  ...
  it(`instructor can revoke an invite code from their classroom`, async () => {
    const response = await request(app)
      .post("/graphql")
      .set("Authorization", `Bearer ${instructorAccessToken}`)
      .send({
        query: revokeClassInviteCodeQuery,
        variables: {
          classId: classId,
          classroomCode: testInviteCode,
        },
      });
    ...
    expect(
      response.body.data.revokeClassInviteCode.inviteCodes
    ).to.have.lengthOf(0);
  });
