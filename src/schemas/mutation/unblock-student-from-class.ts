  ...
  resolve: async (
    ...
    try {
      ...
      if (classMembership.status !== ClassMembershipStatus.BLOCKED) {
        throw new Error("Student is not blocked from this classroom");
      }

      // Set students ClassMembership status to NONE
      classMembership.status = ClassMembershipStatus.NONE;
...
export default unblockStudentFromClass;
